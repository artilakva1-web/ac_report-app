...
with st.sidebar:
    ...
    # --- დამატებული თავმჯდომარის ხელფასი ---
...
if uploaded_file:
    try:
        ...
        # --- გამოთვლები (20%-ის გათვალისწინებით) ---
        raw_collection = (total_residents - debtors_count) * tariff
        net_collection = raw_collection * 0.8  # შემოსავალს გამოკლებული 20%
        
        total_available = previous_balance + net_collection
        
        # ბიუჯეტიდან აკლდება დარიცხული ხელფასი (ხელზე ასაღები + საშემოსავლო)
        final_monthly_balance = total_available - expenses - manager_salary
        
        # ინფორმაციისთვის: ხელზე ასაღები ხელფასი
        ...
        if st.button("🚀 დააგენერირე PDF ანგარიში", type="primary"):
            ...
            # ხელფასის გრაფა ემატება მხოლოდ თუ 0-ზე მეტია
            if manager_salary > 0:
                summary_data.append(["თავმჯდომარის ხელფასი (დარიცხული)", f"{manager_salary:.2f} GEL"])
                summary_data.append(["ხელზე ასაღები (წმინდა -20%)", f"{net_salary:.2f} GEL"])
